# External asset sources

These are the online assets currently copied into `assets/images/online/`. They are used as illustrative context only. None of the people, artists or organisations below should be presented as endorsing the training service.

## unDraw — Personal Training

- Local file: `assets/images/online/undraw-personal-training.svg`
- Source page: https://undraw.co/illustration/personal-training_jq54
- Creator: Katerina Limpitsouni / unDraw
- Licence: unDraw's current licence permits free personal and commercial use without attribution, subject to its restrictions: https://undraw.co/license
- Intended use: a visual cue for controlled movement. It is not a picture of a hypopressive exercise.

## unDraw — Posture and breathing cues

### Yoga

- Local file: `assets/images/online/undraw-yoga.svg`
- Source page: https://undraw.co/illustration/yoga_i399
- Creator: Katerina Limpitsouni / unDraw
- Licence: unDraw's current licence permits free personal and commercial use without attribution, subject to its restrictions: https://undraw.co/license
- Intended use: a visual cue for finding a clear, understood position. It is not a hypopressive instruction.

### Meditation

- Local file: `assets/images/online/undraw-meditation.svg`
- Source page: https://undraw.co/illustrations/5
- Creator: Katerina Limpitsouni / unDraw
- Licence: unDraw's current licence permits free personal and commercial use without attribution, subject to its restrictions: https://undraw.co/license
- Intended use: a visual cue for calm breathing and manageable pacing. It is not a breathing instruction. The page also uses it as a small cue beside the second awareness question.

### A moment to relax

- Local file: `assets/images/online/undraw-a-moment-to-relax.svg`
- Source page: https://undraw.co/illustrations/23
- Creator: Katerina Limpitsouni / unDraw
- Licence: unDraw's current licence permits free personal and commercial use without attribution, subject to its restrictions: https://undraw.co/license
- Intended use: a small, calm visual cue beside the final awareness question. It is not a medical illustration.

## unDraw — Pathway steps

### Book reading

- Local file: `assets/images/online/undraw-book-reading.svg`
- Source page: https://undraw.co/illustration/book-reading_i0eb
- Creator: Katerina Limpitsouni / unDraw
- Licence: unDraw's current licence permits free personal and commercial use without attribution, subject to its restrictions: https://undraw.co/license
- Intended use: the pathway step for learning the basics.

### Check Boxes

- Local file: `assets/images/online/undraw-check-boxes.svg`
- Source page: https://undraw.co/illustration/check-boxes_x5fg
- Creator: Katerina Limpitsouni / unDraw
- Licence: unDraw's current licence permits free personal and commercial use without attribution, subject to its restrictions: https://undraw.co/license
- Intended use: the pathway step for checking personal suitability.

### Medical care

- Local file: `assets/images/online/undraw-medical-care.svg`
- Source page: https://undraw.co/illustration/medical-care_7m9g
- Creator: Katerina Limpitsouni / unDraw
- Licence: unDraw's current licence permits free personal and commercial use without attribution, subject to its restrictions: https://undraw.co/license
- Intended use: the pathway step for seeking healthcare advice when relevant. It is not a clinical assessment.
- Also used as a small, general care cue beside the third awareness question. It is not a clinical assessment.

### Booking

- Local file: `assets/images/online/undraw-booking.svg`
- Source page: https://undraw.co/illustration/booking_8vl5
- Creator: Katerina Limpitsouni / unDraw
- Licence: unDraw's current licence permits free personal and commercial use without attribution, subject to its restrictions: https://undraw.co/license
- Intended use: the pathway step for booking guided training when the route is confirmed.

## Pexels — breathing pause

- Local file: `assets/images/online/pexels-thirdman-breathing-pause.jpg`
- Source page: https://www.pexels.com/photo/woman-in-black-active-wear-sitting-on-a-yoga-mat-while-concentrating-6958254/
- Photographer: Thirdman
- Licence: Pexels' current licence permits free personal and commercial use without required attribution, with restrictions including not implying endorsement: https://www.pexels.com/license/
- Intended use: a calm, illustrative image beside the symptom-awareness signpost. It is not a demonstration of hypopressive technique or a medical image.

## OpenStax — Muscles of the Pelvic Floor

- Local file: `assets/images/online/openstax-pelvic-floor-muscles.jpg`
- Source page: https://commons.wikimedia.org/wiki/File:1115_Muscles_of_the_Pelvic_Floor.jpg
- Original source: OpenStax Anatomy and Physiology, image 1115
- Creator: OpenStax
- Licence: Creative Commons Attribution 4.0 International (CC BY 4.0): https://creativecommons.org/licenses/by/4.0/
- Intended use: general pelvic-floor anatomy context. The image does not diagnose a condition or show a hypopressive exercise.

## Review before launch

- Recheck each source page and licence before publication.
- Keep the source links and attribution visible where required.
- Check that the image context does not imply endorsement, clinical assessment or a guaranteed outcome.
- Confirm that the accompanying health and suitability wording has been reviewed by the service owner and, where appropriate, a qualified healthcare professional.
